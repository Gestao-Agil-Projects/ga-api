...
class ProfessionalService:
    ...
    async def create_professional(
        ...
        # specialities = await self.specialty_dao.find_by_ids(
        #     professional_create_request.specialities,
        # )
        # if len(specialities) != len(professional_create_request.specialities):
        #     raise HTTPException(
        #         status_code=status.HTTP_404_NOT_FOUND,
        #         detail="Uma ou mais especialidades nao foram encontradas.",
        #     )
    ...
    async def update_professional(
        ...
        AdminUtils.validate_user_is_admin(admin_user)
        professional:Professional = await self.professional_dao.find_by_id(professional_id)
        if not professional:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
