...
async def test_creation(
    ...
    url = fastapi_app.url_path_for("create_speciality_model")
    test_name = uuid.uuid4().hex
    response = await client.put(url, json={"title": test_name, "age": 42})
...
async def test_getting_list(
    ...
    speciality1 = Speciality(name=test_name1, age=10)
    speciality2 = Speciality(name=test_name2, age=20)

    await save_and_expect(dao, [speciality1, speciality2], 2)
    ...
    assert specialities[0]["name"] is not None
    assert specialities[1]["name"] is not None
...
async def test_getting_by_id_success(
    ...
    speciality = Speciality(name="by_id", age=77)
    ...
    assert response.status_code == status.HTTP_200_OK
    assert len(data) == 1
    assert data[0]["id"] == speciality.id
...
async def test_deletion_by_id(
    ...
    speciality = Speciality(name="to_delete", age=99)
...
async def test_deletion_by_name(
    ...
    speciality = Speciality(name="to_delete_by_name", age=50)
    ...
    response = await client.delete(
        url,
        params={"speciality_name": "to_delete_by_name"},
        headers={"Authorization": f"Bearer {token}"},
    )
...
async def test_update_speciality_success(
    ...
    speciality = Speciality(name="before_update", age=20)
